import { allColors, allMaterials, type Color, type Material } from "../constants/constants";
...
export function getProductsBySearch(
	...
	try {
		...
		if (colors?.length) {
			filters.push(
				`variants.attributes.color:${colors.map((c) => `"${c}"`).join(",")}`,
			);
		}
		if (materials?.length) {
			filters.push(
...
export async function getCategoryProductsFacets(
	...args: Parameters<typeof getProductsBySearch>
) {
